...
void KeyboardListener::drop(char key){
    ...
    if (pack.size() - 1 < pos ){
        ...
        dropItem(player->getXPos(), player->getYPos(), pack[pos]);
...
void KeyboardListener::dropItem(int X,int Y,Item* target){
    std::string msg;
...
void KeyboardListener::addItem(){
    ...
    if(c == ']'){
        ...
        for(int i = 0; i< (int)armors.size(); i++){
            if(armors[i]->getXPos() == cX && armors[i]->getYPos() == cY){
                item1 = (Item*)armors[i];
            }
        }
    }
        ...
        for(int i = 0; i < (int)swords.size(); i++){
            if(swords[i]->getXPos() == cX && swords[i]->getYPos() == cY){
                item1 = (Item*)swords[i];
            }
        }
    }
        ...
        for (int i = 0; i < (int) scrollVector.size(); i++) {
            if (scrollVector[i]->getXPos() == cX && scrollVector[i]->getYPos() == cY) {
    ...
    if (item1 == NULL){
        ...
        if (scrollAction->getName() == "BlessArmor" || scrollAction->getName() == "BlessSword"){
            ...
            if (cursedItem != NULL){
                ...
                msg = "Info: scroll of cursing does nothing because " + cursedItem->getName() + " not begin used";
